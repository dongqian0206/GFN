...
def main():
    ...
    aggregated_stats_1 = get_stats(
        modes_1, path=glob.glob('D:/saved_models/grid_nvidia_v100/grid_2_64/grid_0.1/*/*/out.pkl')
    )
    aggregated_stats_2 = get_stats(
        modes_2, path=glob.glob('D:/saved_models/grid_nvidia_v100/grid_2_64/grid_0.01/*/*/out.pkl')
    )
    aggregated_stats_3 = get_stats(
        modes_3, path=glob.glob('D:/saved_models/grid_nvidia_v100/grid_2_64/grid_0.001/*/*/out.pkl')
    )
    ...
    for i, data in enumerate([aggregated_stats_1, aggregated_stats_2, aggregated_stats_3]):
        for j, (key, stat) in enumerate(data.items()):
            ...
            axs[1, i].set_xlim([10 ** 5, 10 ** 6])
            axs[1, i].set_ylim([2 * 10 ** -4, 3 * 10 ** -4])
    ...
    plt.show()
    # plt.savefig('./out_2.png', bbox_inches='tight', format='png', dpi=300)
