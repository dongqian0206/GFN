...
def main():
    ...
    for step in range(1, args.num_steps + 1):
        ...
        parent_states, parent_actions, induced_states, rewards, finishes = map(
            torch.cat, zip(*trajectories)
        )

        # in_flow: log F(s_{t}) = log \sum_{s' in Parent(s_{t})} exp( F(s' --> s_{t}) )
        # F(s' --> s_{t}) := F(s', a'), where s_{t} = T(s', a')
        parent_flow = model(get_one_hot(parent_states, h).view(parent_states.size(0), -1))
        ...
        children_flow = model(get_one_hot(induced_states, h).view(induced_states.size(0), -1))
        children_mask = get_mask(induced_states, h)
        children_flow = (children_flow - 1e10 * children_mask)
        children_flow = children_flow * (1 - finishes).unsqueeze(1) - 1e10 * finishes.unsqueeze(1)
